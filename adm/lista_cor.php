            ...
            <div class="span12">
                <div class="box box-color box-bordered">
                    <div class="box-title">
                        ...
                        <div class="actions">
                            <a href="cad_cor.php" class="btn" rel="tooltip" title="Cadastrar Cor"><i
                                        class="icon-save"></i></a>
                        </div>
                    </div>
                    <div class="box-content nopadding">
                        <table class="table table-hover table-nomargin dataTable table-bordered dataTable-buttonTools">
                            ...
                            <tbody>
                            <?php
                            $sql = "SELECT * FROM cor ORDER BY descricao";
                            $res = mysqli_query($conectar, $sql);
                            while ($row = mysqli_fetch_assoc($res)) {
                                ?>
                                <tr>
                                    <td><?php echo utf8_encode($row['descricao']); ?></td>
                                    <td><?php exibeData($row['dat_cadastro']); ?></td>
                                    <td><?php echo $row['hor_cadastro']; ?></td>
                                    <td><?php echo utf8_encode($row['responsavel']); ?></td>
                                    <td>
                                        <a href="alt_cor.php?var_cor=<?php echo $row['id']; ?>" class="btn"
                                           rel="tooltip" title="Editar"><i class="icon-edit"></i></a>
                                        <a href="#"
                                           onClick="confirmDelete('exclui_cor.php?var_cor=',<?php echo $row['id']; ?>)"
