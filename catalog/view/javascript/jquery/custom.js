...
$(".quantity-adder .add-action").click( function(){
	if( $(this).hasClass('add-up') ) {  
		$("[name=itemQuantity]",'.quantity-adder').val( parseInt($("[name=itemQuantity]",'.quantity-adder').val()) + 1 );
	}else {
		if( parseInt($("[name=itemQuantity]",'.quantity-adder').val())  > 1 ) {
			$("input",'.quantity-adder').val( parseInt($("[name=itemQuantity]",'.quantity-adder').val()) - 1 );
		}
	}
} );